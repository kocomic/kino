...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    go build -trimpath -ldflags="-s -w" -o /out/kino ./cmd/kino
RUN ./scripts/collect-third-party-licenses.sh /out/third-party-licenses --go-only

FROM alpine:3.23@sha256:fd791d74b68913cbb027c6546007b3f0d3bc45125f797758156952bc2d6daf40
LABEL org.opencontainers.image.title="Kino" \
      org.opencontainers.image.description="Self-hosted ROM library management service" \
      org.opencontainers.image.licenses="Apache-2.0"
RUN apk add --no-cache ca-certificates tzdata \
    && addgroup -S -g 10001 kino \
    && adduser -S -D -H -u 10001 -G kino kino \
    && mkdir -p /data /library \
    && chown -R kino:kino /data /library

COPY --from=build /out/kino /usr/local/bin/kino
COPY --from=build /out/third-party-licenses /usr/share/doc/kino/THIRD_PARTY_LICENSES
COPY LICENSE /usr/share/licenses/kino/LICENSE
