...
HashPack is Kino's privacy-minimized, shareable ROM identity data format. It
...
`source.license` is a required dataset-license declaration; Kino checks its
...
`application/vnd.kino.hashpack+zip`. It MUST contain exactly two regular file
...
The first field is the frozen v1 identity salt defined by
[`IdentityDomainV1`](../internal/hashpack/hashpack.go), retained across the Kino rename.
It is not the current product name; changing it would invalidate existing pack IDs.
...
Kino binds a preview token to the exact uploaded pack bytes. Commit requires
...
a playable library entry: Kino still requires a locally available ROM whose
