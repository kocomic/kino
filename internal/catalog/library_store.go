...
func (s *Store) DeleteEdition(ctx context.Context, id string) error {
	...
	defer tx.Rollback()
	if err = protectLegacyEditions(ctx, tx, `SELECT id FROM editions WHERE id=?`, id); err != nil {
		return err
	}
	var gameID string
...
func (s *Store) DeleteGame(ctx context.Context, id string) error {
	...
	defer tx.Rollback()
	if err = protectLegacyEditions(ctx, tx, `SELECT id FROM editions WHERE game_id=?`, id); err != nil {
		return err
	}
