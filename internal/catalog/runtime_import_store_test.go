...
func TestRuntimeImportHintFailureRollsBackWholeBatch(t *testing.T) {
	...
	err := s.ImportGamesAtomic(context.Background(), []ImportedGame{
		...
		{Platform: "gba", DefaultTitle: "Second", EditionTitle: "Original", EditionType: "original", Artifacts: []NewArtifact{{Path: "gba/second-hint.gba", SHA256: "second-hint"}}, RuntimeHints: []NewRuntimeImportHint{{SourceKind: "structured-sidecar", SourceFormat: "kino-launches-v1", DriverID: "bad id with spaces"}}},
	})
	if err == nil {
