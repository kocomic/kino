...
func normalizeSourceInput(in NewLibrarySource) (NewLibrarySource, error) {
	...
	if in.Kind != "rom_directory" && in.Kind != "pegasus" && in.Kind != "esde" && in.Kind != "kino" {
		return in, errors.New("kind must be rom_directory, pegasus, esde, or kino")
	}
	if in.SourceAdapterID == "" {
		in.SourceAdapterID = BuiltinSourceAdapterID(in.Kind)
	}
	if in.Platform == "" && in.Kind != "kino" {
		return in, errors.New("platform is required unless kind is kino")
	}
