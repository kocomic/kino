...
func TestMigrationFromV12AllowsNeutralSourcesAndPreservesScanHistory(t *testing.T) {
	...
	if _, err = db.Exec(`
		...
		SET sql = replace(sql, '''rom_directory'',''pegasus'',''esde'',''kino''', '''rom_directory'',''pegasus'',''esde''')
	...
	neutral, err := migrated.CreateLibrarySource(ctx, NewLibrarySource{Name: "Multi-platform recovery", Kind: "kino", MetadataPath: "recovery/library-manifest.json", ROMStoragePolicy: "reference", MediaStoragePolicy: "copy"})
	if err != nil || neutral.Kind != "kino" || neutral.Platform != "" || neutral.RootPath != "recovery" {
...
func TestMigrationFromV13AddsSourceAdaptersAndPreservesSources(t *testing.T) {
	...
	for _, adapter := range adapters {
		...
		if adapter.ID == "builtin-source-direct-rom" {
			expectedContract = 3
		} else if adapter.ID == "builtin-source-kino" {
			expectedContract = 4
		}
...
func TestMigrationFromV25NormalizesPreviewNeutralNamespace(t *testing.T) {
	...
	source, err := store.CreateLibrarySource(ctx, NewLibrarySource{
		Name: "Preview neutral source", Kind: "kino", MetadataPath: "recovery/library-manifest.json",
	...
	if _, err = db.Exec(`
		...
		PRAGMA writable_schema = ON;
		UPDATE sqlite_master
		SET sql = replace(sql, '''rom_directory'',''pegasus'',''esde'',''kino''', '''rom_directory'',''pegasus'',''esde'',''legacy-neutral''')
	...
	gotSource, err := migrated.GetLibrarySource(ctx, source.ID)
	if err != nil || gotSource.Kind != "kino" || gotSource.SourceAdapterID != "builtin-source-kino" {
...
func TestMigrationFromV27RenamesProductNamespace(t *testing.T) {
	...
	source, err := store.CreateLibrarySource(ctx, NewLibrarySource{
		Name: "Recovery source", Kind: "kino", MetadataPath: "recovery/library-manifest.json",
		ROMStoragePolicy: "reference", MediaStoragePolicy: "copy",
	})
	if err != nil {
		t.Fatal(err)
	...
	canonicalConstraint := "'rom_directory','pegasus','esde','kino'"
	...
	// Execute parameterized statements separately: SQLite binds each statement
	// independently, so a multi-statement Exec can leave the source unchanged.
	if _, err = db.Exec(`UPDATE source_adapters SET id=?,format=?,handler=? WHERE id='builtin-source-kino'`, legacyAdapterID, legacyProductName, legacyProductName); err != nil {
		db.Close()
		t.Fatal(err)
	}
	if _, err = db.Exec(`UPDATE library_sources SET kind=?,source_adapter_id=? WHERE id=?`, legacyProductName, legacyAdapterID, source.ID); err != nil {
		db.Close()
		t.Fatal(err)
	}
