	...
	for _, platform := range platforms {
		...
		for _, w := range byPlatform[platform] {
			for _, e := range w.Editions {
				...
				fmt.Fprintf(&b, "x-kino-game-id: %s\nx-kino-edition-id: %s\nx-kino-game-title: %s\nx-kino-edition-title: %s\nx-kino-edition-type: %s\n", w.ID, e.ID, w.DefaultTitle, e.DefaultTitle, e.EditionType)
				if e.Version != "" {
					fmt.Fprintf(&b, "x-kino-version: %s\n", e.Version)
				}
				if e.Author != "" {
					fmt.Fprintf(&b, "x-kino-author: %s\n", e.Author)
				}
				if len(e.Languages) > 0 {
					fmt.Fprintf(&b, "x-kino-languages: %s\n", strings.Join(e.Languages, ", "))
				}
				for l, t := range w.Titles {
					fmt.Fprintf(&b, "x-kino-game-title-%s: %s\n", l, t)
				}
				for l, t := range e.Titles {
					fmt.Fprintf(&b, "x-kino-edition-title-%s: %s\n", l, t)
				}
				b.WriteString("\n")
