...
const launchManifestName = "kino-launches.json"
...
func findLaunchManifest(libraryRoot, metadataPath string) (string, error) {
	path, err := findAncestorFile(libraryRoot, metadataPath, launchManifestName, maxLaunchManifestSize)
	if err != nil || path != "" {
		return path, err
	}
	return findAncestorFile(libraryRoot, metadataPath, "varkiv-launches.json", maxLaunchManifestSize)
}
...
func attachStructuredLaunchHints(libraryRoot, metadataPath string, games []catalog.ImportedGame) ([]catalog.ImportedGame, error) {
	...
	for _, binding := range manifest.Bindings {
		...
		hint := catalog.NewRuntimeImportHint{SourceKind: "structured-sidecar", SourceFormat: fmt.Sprintf("kino-launches-v%d", manifest.FormatVersion), DeviceProfileID: deviceID, FrontendAdapterID: frontendID, DriverID: driverID, CoreID: coreID, Arguments: binding.Binding.Arguments, SourceRef: sourceRef}
