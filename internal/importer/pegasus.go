...
func PreviewPegasusWithContentRoot(libraryRoot, metadataPath, contentRoot, platform, locale string) ([]catalog.ImportedGame, error) {
	...
	// Prefer explicit Kino fields; accept archived exports without changing their files.
	for i := range entries {
		for key, values := range entries[i].values {
			if strings.HasPrefix(key, "x-varkiv-") {
				current := "x-kino-" + strings.TrimPrefix(key, "x-varkiv-")
				if _, exists := entries[i].values[current]; !exists {
					entries[i].values[current] = values
				}
			}
		}
	}
	...
	for _, entry := range entries {
		...
		for key, values := range entry.values {
			if strings.HasPrefix(key, "x-kino-game-title-") && len(values) > 0 {
				titles[strings.TrimPrefix(key, "x-kino-game-title-")] = values[0]
			}
			if strings.HasPrefix(key, "x-kino-edition-title-") && len(values) > 0 {
				editionTitles[strings.TrimPrefix(key, "x-kino-edition-title-")] = values[0]
			}
		}
		langs := splitCSV(first(entry, "x-kino-languages"))
		editionType := first(entry, "x-kino-edition-type")
		if editionType == "" {
			editionType = "original"
		}
		gameID := first(entry, "x-kino-game-id")
		editionID := first(entry, "x-kino-edition-id")
		gameTitle := first(entry, "x-kino-game-title")
		if gameTitle == "" {
			gameTitle = title
		}
		editionTitle := first(entry, "x-kino-edition-title")
		if editionTitle == "" {
			editionTitle = title
		}
