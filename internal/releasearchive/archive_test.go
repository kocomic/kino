...
func TestWriteIsDeterministicAcrossSourceMetadata(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(source, "kino"), []byte("binary\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	for _, format := range []Format{FormatZip, FormatTarGz} {
		t.Run(string(format), func(t *testing.T) {
			...
			if err := Write(source, "kino-test", first, format); err != nil {
				t.Fatal(err)
			}
			future := time.Date(2037, 7, 9, 13, 12, 11, 0, time.UTC)
			for _, target := range []string{source, filepath.Join(source, "kino"), filepath.Join(source, "licenses"), filepath.Join(source, "licenses", "NOTICE")} {
				if err := os.Chtimes(target, future, future); err != nil {
					t.Fatal(err)
				}
			}
			if err := Write(source, "kino-test", second, format); err != nil {
				t.Fatal(err)
			}
			...
			if names := archiveNames(t, first, format); !reflect.DeepEqual(names, []string{"kino-test/", "kino-test/kino", "kino-test/licenses/", "kino-test/licenses/NOTICE"}) {
...
func TestWriteRefusesOverwriteAndUnsafeInputs(t *testing.T) {
	...
	if err := Write(source, "kino-test", output, FormatZip); err == nil || err.Error() != "release archive output already exists" {
	...
	if err := Write(source, "kino-test", filepath.Join(source, "inside.zip"), FormatZip); err == nil {
...
func TestWriteRejectsSymbolicLinks(t *testing.T) {
	...
	output := filepath.Join(t.TempDir(), "release.zip")
	if err := Write(source, "kino-test", output, FormatZip); err == nil {
...
func assertArchiveMetadata(t *testing.T, archivePath string, format Format) {
	...
	wantModes := map[string]os.FileMode{
		"kino-test/":                0o755,
		"kino-test/licenses/":       0o755,
		"kino-test/licenses/NOTICE": 0o644,
		"kino-test/kino":            executableMode,
	}
	switch format {
	case FormatZip:
		reader, err := zip.OpenReader(archivePath)
