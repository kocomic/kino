...
func (s *Server) exportHashPack(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/vnd.kino.hashpack+zip")
	w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="%s"`, filename))
	w.Header().Set("X-Kino-Pack-ID", manifest.PackID)
	w.Header().Set("X-Kino-Record-Count", fmt.Sprintf("%d", manifest.RecordCount))
