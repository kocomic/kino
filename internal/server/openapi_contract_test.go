	...
	for _, fixture := range openAPIContracts() {
		t.Run(fixture.name, func(t *testing.T) {
			...
			if fixture.name == "library" {
				...
				required = append(required,
					"title: Kino Multiplayer Coordination API",
					"summary: Stable coordination contract for emulator-aware multiplayer clients",
					"version: v1",
					"X-Kino-Multiplayer-Version: {$ref: '#/components/headers/MultiplayerVersion'}",
					"example:",
				)
			}
			...
			if fixture.name == "multiplayer" {
				if got := strings.Count(contract, "X-Kino-Multiplayer-Version: {$ref: '#/components/headers/MultiplayerVersion'}"); got != 8 {
