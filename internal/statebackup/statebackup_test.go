...
func makeFixture(t *testing.T) fixture {
	...
	_, err = store.AddEditionWithArtifact(ctx, catalog.NewEdition{GameID: game.ID, DefaultTitle: "Original", EditionType: "original"}, catalog.NewArtifact{
	...
	// A synthetic legacy row proves that a ROM-only upgrade still backs up
	// old user data without shipping the removed synchronization implementation.
	save := []byte("synthetic-save")
	checksum := hashBytes(save)
	blobPath := filepath.Join(state, "saves", "blobs", checksum[:2], checksum)
	if err = os.MkdirAll(filepath.Dir(blobPath), 0700); err != nil {
		t.Fatal(err)
	}
	if err = os.WriteFile(blobPath, save, 0600); err != nil {
		t.Fatal(err)
	}
	...
	for _, statement := range []string{
		`INSERT INTO devices(id,name,os_family,created_at,updated_at,last_seen_at) VALUES('legacy-device','Legacy','linux','2026-01-01','2026-01-01','2026-01-01')`,
		`INSERT INTO save_streams(id,namespace,owner_type,owner_key,created_at,updated_at) VALUES('legacy-stream','legacy','container','legacy','2026-01-01','2026-01-01')`,
		`INSERT INTO save_revisions(id,stream_id,device_id,content_hash,total_size,file_count,status,created_at) VALUES('legacy-revision','legacy-stream','legacy-device','legacy-hash',14,1,'current','2026-01-01')`,
	} {
		if _, err = legacy.Exec(statement); err != nil {
			t.Fatal(err)
		}
	}
	if _, err = legacy.Exec(`INSERT INTO save_files(id,revision_id,logical_path,checksum,size,blob_path) VALUES('legacy-file','legacy-revision','game.srm',?,?,?)`, checksum, len(save), blobPath); err != nil {
		t.Fatal(err)
	}
	...
	closed = true
	return fixture{database: database, state: state, gameID: game.ID, revisionID: "legacy-revision", fileID: "legacy-file", rom: rom, media: media, save: save}
}
...
func TestCompleteStateBackupCheckAndRestoreRoundTrip(t *testing.T) {
	...
	restoredDB, err := sql.Open("sqlite", filepath.Join(restored, databaseName))
	if err != nil {
		t.Fatal(err)
	}
	defer restoredDB.Close()
	var blob string
	if err = restoredDB.QueryRow(`SELECT blob_path FROM save_files WHERE id=?`, fixture.fileID).Scan(&blob); err != nil {
		t.Fatal(err)
	}
