...
temporary="$(mktemp "$repo_root/bin/.kino-new.XXXXXX")"
cleanup() {
  rm -f "$temporary"
}
trap cleanup EXIT

go build -trimpath -o "$temporary" ./cmd/kino
identity="$("$temporary" version --json)"
expected="{\"format\":\"kino-version-v1\",\"application_version\":\"$version\"}"
...
trap - EXIT
echo "local_build=passed version=$version output=bin/kino"
