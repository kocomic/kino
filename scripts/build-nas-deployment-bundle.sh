...
output="$repository_root/dist/kino-nas-source-$version.tar.gz"
...
bundle_name="kino-nas-source-$version"
fixture_root=$(mktemp -d "${TMPDIR:-/tmp}/kino-nas-bundle.XXXXXX")
bundle_root="$fixture_root/$bundle_name"

cleanup() {
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/kino-nas-bundle.*) rm -rf -- "$fixture_root" ;;
