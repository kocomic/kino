...
const packID = crypto.createHash('sha256').update([
  'varkiv-hashpack-v1', // Frozen v1 identity salt; matches the Go implementation.
  manifest.source.id,
...
"${ajv[@]}" validate --spec=draft2020 \
  -s schemas/kino-launches-v2.schema.json \
  -d testdata/portable-runtime-v2/kino-launches.json \
  -d testdata/portable-standalone-v2/kino-launches.json \
  -d testdata/kino-launches.json
