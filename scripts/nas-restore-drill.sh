...
usage() {
  printf '%s\n' 'usage: scripts/nas-restore-drill.sh --backup SAFE_NAME [--name SAFE_NAME] [--env-file FILE] [--project-name NAME]'
}
...
backup_root=$(nas_env_value "$env_file" KINO_BACKUP_PATH)
restore_root=$(nas_env_value "$env_file" KINO_RESTORE_PATH)
data_root=$(nas_env_value "$env_file" KINO_DATA_PATH)
nas_require_absolute_directory KINO_BACKUP_PATH "$backup_root"
nas_require_absolute_directory KINO_RESTORE_PATH "$restore_root"
nas_require_absolute_directory KINO_DATA_PATH "$data_root"
