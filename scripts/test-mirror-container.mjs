...
const source = `ghcr.io/example/kino@${digest}`;
const target = 'docker.io/example/kino';
...
function fixture(t) {
  const root = mkdtempSync(join(tmpdir(), 'kino-mirror-test.'));
...
test('rejects mutable sources, unsupported tags, and injected arguments', t => {
  ...
  assert.equal(f.run(undefined, {}, 'ghcr.io/example/kino:edge').status, 2);
  assert.equal(f.copies().length, 0);
});
