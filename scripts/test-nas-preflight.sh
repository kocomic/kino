...
fixture_root=$(mktemp -d "${TMPDIR:-/tmp}/kino-nas-preflight-test.XXXXXX")

cleanup() {
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/kino-nas-preflight-test.*) rm -rf -- "$fixture_root" ;;
...
write_env() {
  ...
  printf '%s\n' \
    ...
    "KINO_DATA_PATH=$data_path" \
    "KINO_BACKUP_PATH=$backup_path" \
    "KINO_RESTORE_PATH=$restore_path" \
    "GAME_LIBRARY_TOKEN=$token" \
    'KINO_BIND=127.0.0.1' \
    'KINO_PORT=18086' > "$target"
}
...
if KINO_TEST_FILESYSTEM=nfs PATH="$fixture_root/bin:$PATH" "$repository_root/scripts/nas-preflight.sh" --env-file "$network_env" >/dev/null 2>&1; then
