...
fixture_root=$(mktemp -d "${TMPDIR:-/tmp}/kino-container-compose.XXXXXX")
stage=requirements
...
cleanup() {
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/kino-container-compose.*) rm -rf -- "$fixture_root" ;;
    *) printf '%s\n' 'error: refusing to clean unexpected fixture path' >&2 ;;
  esac
}
trap 'report_failure; cleanup' EXIT

tag_image='ghcr.io/example/kino:edge'
digest_image='ghcr.io/example/kino@sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'

stage=tag-render
"$renderer" --image "$tag_image" --out "$fixture_root/tag.yaml" >/dev/null
grep -Fq "image: \"$tag_image\"" "$fixture_root/tag.yaml"
! grep -Fq 'KINO_IMAGE' "$fixture_root/tag.yaml"
...
KINO_DATA_PATH="$fixture_root/data" \
...
if "$renderer" --image 'ghcr.io/Example/kino:edge' --out "$fixture_root/uppercase.yaml" >/dev/null 2>&1; then
  printf '%s\n' 'renderer accepted an uppercase registry path' >&2
  exit 1
fi
if "$renderer" --image $'ghcr.io/example/kino:edge\nservices: {}' --out "$fixture_root/injection.yaml" >/dev/null 2>&1; then
