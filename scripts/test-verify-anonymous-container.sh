...
test_root="$(mktemp -d "${TMPDIR:-/tmp}/kino-anonymous-container-test.XXXXXX")"
chmod 700 "$test_root"
cleanup() {
  case "$test_root" in
    "${TMPDIR:-/tmp}"/kino-anonymous-container-test.*) rm -rf -- "$test_root" ;;
...
printf '%s\n' '#!/usr/bin/env bash' \
  ...
  'printf "%s\n" "${FAKE_OUTPUT:-Kino 0.1.0-preview.5}"' >"$fake_docker"
...
image_ref="ghcr.io/kocomic/kino@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
run_verifier() {
  ...
  PATH="$fake_bin:$PATH" \
    ...
    FAKE_FAILURES="$1" \
    KINO_ANONYMOUS_PULL_ATTEMPTS=3 \
    KINO_ANONYMOUS_PULL_DELAY_SECONDS=0 \
    "${verifier_command[@]}"
}
...
set +e
KINO_ANONYMOUS_PULL_ATTEMPTS=31 \
  "$repository_root/scripts/verify-anonymous-container.sh" \
    --image "$image_ref" --version 0.1.0-preview.5 >/dev/null 2>&1
invalid_attempts_status=$?
KINO_ANONYMOUS_PULL_DELAY_SECONDS=61 \
  "$repository_root/scripts/verify-anonymous-container.sh" \
    --image "$image_ref" --version 0.1.0-preview.5 >/dev/null 2>&1
