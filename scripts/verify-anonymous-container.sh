...
immediately. KINO_ANONYMOUS_PULL_ATTEMPTS and
KINO_ANONYMOUS_PULL_DELAY_SECONDS may tighten that fixed bound.
...
attempts="${KINO_ANONYMOUS_PULL_ATTEMPTS:-20}"
delay_seconds="${KINO_ANONYMOUS_PULL_DELAY_SECONDS:-30}"
if [[ ! "$attempts" =~ ^[0-9]+$ ]] || ((attempts < 1 || attempts > 30)); then
  echo "error: KINO_ANONYMOUS_PULL_ATTEMPTS must be between 1 and 30" >&2
  exit 2
fi
if [[ ! "$delay_seconds" =~ ^[0-9]+$ ]] || ((delay_seconds > 60)); then
  echo "error: KINO_ANONYMOUS_PULL_DELAY_SECONDS must be between 0 and 60" >&2
  exit 2
fi
command -v docker >/dev/null 2>&1 || { echo "error: docker is unavailable" >&2; exit 2; }

diagnostic_log="$(mktemp "${TMPDIR:-/tmp}/kino-anonymous-pull.XXXXXX")"
anonymous_config="$(mktemp -d "${TMPDIR:-/tmp}/kino-anonymous-config.XXXXXX")"
cleanup() {
  rm -f -- "$diagnostic_log"
